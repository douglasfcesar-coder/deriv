...
export function useDerivWS(token) {
  ...
  const addTrade = useCallback((newTrade) => {
    setTrades(prev => {
      if (prev.find(t => t.id === newTrade.id)) return prev
  ...
  const connect = useCallback(() => {
    ...
    socket.onmessage = (e) => {
      ...
      if (type === 'authorize') {
        ...
        // 1. Subscribe to real-time balance
        socket.send(JSON.stringify({ balance: 1, subscribe: 1 }))

        // 2. Subscribe to real-time transactions (new trades as they close)
        socket.send(JSON.stringify({ transaction: 1, subscribe: 1 }))

        // 3. Load historical trades
        fetchHistory(socket)

        // 4. Polling fallback every 30s to catch anything missed
      ...
      // Real-time transaction event — fires every time a contract closes
      if (type === 'transaction' && data.transaction) {
        const tx = data.transaction
        // Only process sell (contract closed) events
        if (tx.action === 'sell' && tx.amount != null) {
          ...
          // Also update balance after each trade
      ...
      if (type === 'profit_table' && data.profit_table) {
        ...
        setTrades(prev => {
          ...
          const newOnes = txs.filter(t => !existingIds.has(t.id))
...
export function getMockData() {
  const types = ['RUNS R_10', 'RUNS R_10', 'RUNS R_10', 'RUNS R_10', 'RUNS R_10', 'RUNS R_10']
